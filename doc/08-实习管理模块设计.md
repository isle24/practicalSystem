...
internship_plan（实习课程计划）
  ...
  - category_id（实习类别，决定归属维度）
  - grade_id（年级，非毕业实习使用）
  - graduation_cohort_id（毕业届次，毕业实习使用）
  - dep_id, profession_id（学院、专业）
  - credit（课程学分）
  - student_count（兼容统计字段，不作为计划录入项）
...
1. 实习计划生成
   ...
   - 计划按专业制定，包含课程、学分、实习类别、实际归属范围、学院和专业等基础信息。
   - 毕业实习的实际归属范围为毕业届次，其他实习为年级；计划主表不录入学生人数。
...
| 字段 | 必填 | 说明 |
|---|---|---|
| 实习类别 | 是 | 匹配 internship_category，并决定归属维度 |
| 年级/毕业届次 | 是 | 非毕业实习匹配 grade_list，毕业实习匹配 graduation_cohort |
| 学院 | 是 | 匹配 department |
| 专业 | 是 | 匹配 profession |
| 课程代码 | 否 | 从教务系统抽取时使用 |
...
```
当前业务归属及上一业务归属：保留在当前学校业务库，在线查询
超过两个归属周期的数据：按年归档到当前学校业务库 recording_archive_YYYY 表
归档后原表数据物理删除，减少业务库体积
归档数据仍可在 PC 端"历史归档"页面查询（查询走当前学校业务库归档表，始终不跨学校库）
```

归档定时任务：按学校配置执行，默认在 7 月、12 月每日凌晨分批执行。
...
`internship_brief` 按 `week_key + scope_type + scope_id` 保存学校、学院和专业范围的周报快照，学院和专业管理员只能接收并查看本组织范围数据。
...
inspection_record（抽检记录）
  - id, uuid
  - grade_id（年级；毕业实习通过任务所属计划读取毕业届次）
...
每个归档周期结束后，系统自动按实际归属范围/课程/任务/类型归档所有电子材料：

```
归档结构：
/{scope_type}/{scope}/course/{course}/task/{task}/type/{type}/
  ├── 实习课程计划表
  ├── 教学实习实施表
  ├── 实习大纲及指导书
  ├── 实习情况及成绩汇总表
