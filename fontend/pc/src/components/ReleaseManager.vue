<template>
  <section class="release-manager"><header><el-radio-group v-model="product" @change="load(1)"><el-radio-button value="system">系统更新</el-radio-button><el-radio-button value="desktop">客户端版本</el-radio-button></el-radio-group><el-button :icon="RefreshCw" :loading="loading" @click="load(page)">刷新</el-button><el-button v-if="product === 'desktop'" :icon="Download" :loading="busy" @click="github">从 GitHub 导入</el-button></header>
...
<script setup>
import { ref } from 'vue';
import { RefreshCw, Download, Copy } from '@lucide/vue';
...
async function importVersion() { if (busy.value) return; busy.value = true; try { selected.value = await request('/release/import', { method: 'POST', body: JSON.stringify({ id: githubId.value }) }); baseline = JSON.stringify(payload()); githubVisible.value = false; tab.value = 'assets'; await load(1); } catch (e) { ElMessage.error(e.message); } finally { busy.value = false; } }
load();
</script>
<style scoped>
.release-manager{height:100%;min-height:0;display:flex;flex-direction:column;gap:16px;padding:22px;box-sizing:border-box}.release-manager>header{display:flex;gap:12px;align-items:center;flex-wrap:wrap}.release-manager>footer{display:flex;align-items:center;justify-content:space-between;color:#77818e;font-size:12px}.release-detail{padding:24px;display:flex;flex-direction:column;gap:20px;overflow:auto;min-height:0}.release-detail label{display:flex;flex-direction:column;gap:10px}.release-detail small{color:#7c8794;line-height:1.6;overflow-wrap:anywhere}.release-sql{display:flex;flex-direction:column;gap:12px}.release-sql pre{padding:18px;background:#f4f6f8;border-radius:5px;overflow:auto;max-height:280px;font:12px/1.8 monospace}.release-sql>div{display:flex;gap:10px}
</style>
