...
async function save() {
  ...
  try {
    ...
    const payload = { ...form.value, request_config: { ...form.value.request_config, form: form.value.request_config.method === 'POST' ? form.value.request_config.form : [] } };
    const { item } = await request('/favorite/save', { method: 'POST', body: JSON.stringify(payload) });
