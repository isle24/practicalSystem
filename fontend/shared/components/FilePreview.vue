<template>
  <div class="file-preview-mask" @click.self="emit('close')">
    <section ref="dialog" class="file-preview" role="dialog" aria-modal="true" :aria-label="name" tabindex="-1" @keydown="keyboard">
      ...
      <div ref="body" class="preview-body" :class="{ 'is-spreadsheet': kind === 'xlsx' }" :aria-busy="loading || rendering">
        ...
        <template v-else>
          ...
          <div v-else-if="kind === 'xlsx'" class="preview-spreadsheet">
            <p v-if="sheets[sheetIndex]?.truncated" class="spreadsheet-notice">仅预览前 2000 行、100 列，请下载查看完整表格。</p>
            <div v-if="sheets[sheetIndex]?.rows.length" class="preview-spreadsheet-scroll">
              <table class="preview-spreadsheet-table">
                <colgroup><col class="preview-row-number-column"><col v-for="(width, column) in sheets[sheetIndex].columnWidths" :key="column" :style="{ width: `${width}px` }"></colgroup>
                <thead><tr><th class="preview-corner" scope="col"></th><th v-for="label in sheets[sheetIndex].columnLabels" :key="label" scope="col">{{ label }}</th></tr></thead>
                <tbody><tr v-for="(row, index) in sheets[sheetIndex].rows" :key="index" :style="{ height: `${sheets[sheetIndex].rowHeights[index]}px` }"><th class="preview-row-number" scope="row">{{ index + 1 }}</th><td v-for="cell in row" :key="cell.key" :rowspan="cell.rowspan" :colspan="cell.colspan" :style="cell.style">{{ cell.text }}</td></tr></tbody>
              </table>
            </div>
            <p v-else class="preview-state">空工作表</p>
          </div>
...
<style scoped>
.file-preview-mask{position:fixed;inset:0;z-index:2147483000;background:rgba(18,24,34,.5);display:grid;place-items:center;padding:20px;box-sizing:border-box}.file-preview{width:min(1180px,100%);height:min(900px,100%);max-height:100%;display:flex;flex-direction:column;overflow:hidden;background:#fff;color:#273243;border-radius:8px;box-shadow:0 16px 60px #0003;outline:none}.file-preview header{display:flex;align-items:center;justify-content:space-between;padding:12px 18px;border-bottom:1px solid #e3e7ed;gap:16px}.file-preview header strong{font-size:15px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.preview-actions,.preview-tools{display:flex;align-items:center;gap:8px}.file-preview button,.preview-actions a{display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;min-width:34px;height:34px;padding:0 8px;border:0;border-radius:5px;background:transparent;color:inherit;cursor:pointer;font:inherit}.file-preview button:hover,.preview-actions a:hover{background:#e9eff8}.file-preview button:disabled{opacity:.35;cursor:default}.preview-tools{padding:8px 14px;border-bottom:1px solid #e3e7ed;flex-wrap:wrap;font-size:12px}.preview-search{display:flex;align-items:center;margin-left:auto;gap:4px}.preview-search input{border:1px solid #d8dee7;border-radius:5px;min-width:0;width:130px;padding:7px;font:inherit}.preview-body{flex:1;min-height:0;overflow:auto;background:#edf0f4;position:relative}.preview-body.is-spreadsheet{background:#fff}.preview-state{padding:48px 24px;text-align:center;font-size:14px}.preview-image{height:100%;display:grid;place-items:center;padding:16px;box-sizing:border-box;overflow:auto}.preview-image img{max-width:100%;max-height:100%;object-fit:contain;transform-origin:center}.preview-body video{display:block;width:100%;height:100%;background:#14171c}.preview-body audio{display:block;width:min(600px,90%);margin:80px auto}.preview-body canvas{display:block;margin:16px auto;background:white;box-shadow:0 1px 6px #0002}.preview-body iframe{width:100%;height:100%;border:0;display:block;background:white}.preview-sheets{display:flex;overflow-x:auto;padding:8px;gap:8px;border-bottom:1px solid #dce2ea}.preview-sheets button{white-space:nowrap;font-size:13px}.preview-sheets .active{color:#136c52;background:#e1f3eb}.preview-spreadsheet{width:max-content;min-width:100%;min-height:100%;background:white;font-size:13px}.preview-spreadsheet-scroll{width:max-content;min-width:100%;padding-bottom:20px}.preview-spreadsheet-table{width:max-content;min-width:100%;border-collapse:collapse;table-layout:fixed;white-space:pre-wrap}.preview-spreadsheet-table th,.preview-spreadsheet-table td{border:1px solid #d8dee7;padding:5px 8px;min-width:48px;max-width:420px;overflow-wrap:anywhere;line-height:1.35}.preview-spreadsheet-table thead th{height:28px;padding:4px 8px;background:#f3f5f7;color:#5b6675;font-weight:500;text-align:center;position:sticky;top:0;z-index:2;white-space:nowrap}.preview-spreadsheet-table .preview-corner{left:0;z-index:4;background:#e8edf2}.preview-spreadsheet-table .preview-row-number-column{width:48px!important}.preview-spreadsheet-table .preview-row-number{width:48px;min-width:48px;padding:4px;background:#f3f5f7;color:#748094;font-weight:400;text-align:center;position:sticky;left:0;z-index:1;white-space:nowrap}.preview-spreadsheet-table td{background:#fff;vertical-align:middle}.spreadsheet-notice{margin:0;padding:10px 12px;color:#6a7380;border-bottom:1px solid #e0e4ea;position:sticky;left:0}.preview-spreadsheet p{padding:12px;color:#6a7380}@media(max-width:600px){.file-preview-mask{padding:0}.file-preview{width:100%;height:100%;height:100dvh;border-radius:0;padding-top:env(safe-area-inset-top);padding-bottom:env(safe-area-inset-bottom);box-sizing:border-box}.file-preview header{padding:12px}.preview-tools{gap:3px;padding:6px}.preview-search{width:100%;margin:0}.preview-search input{flex:1}.preview-body{overscroll-behavior:contain}.preview-spreadsheet-table th,.preview-spreadsheet-table td{padding:5px 6px}}
</style>
